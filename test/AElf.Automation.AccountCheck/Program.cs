...
namespace AElf.Automation.AccountCheck
{
    class Program
    {
        ...
        {
            Log4NetHelper.LogInit("AccountCheck_");
            ...
            if (!check.OnlyCheck)
            {
                ...
                else
                {
                    ...
                    {
                        var token = new TokenContract(transfer.NodeManager, transfer.InitAccount, contract.ContractAddress);
            ...
            else
            {
                ...
                var req = (double)(check.CheckTimes * check.AccountList.Count) / (all / 1000);
