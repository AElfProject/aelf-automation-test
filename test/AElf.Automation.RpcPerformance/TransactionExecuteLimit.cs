...
namespace AElf.Automation.RpcPerformance
{
    public class TransactionExecuteLimit
    {
        ...
        private async Task<ConfigurationContainer.ConfigurationStub> GetConfigurationContractStub()
        {
            ...
            var basicZeroStub =
                _stub.Create<BasicContractZeroContainer.BasicContractZeroStub>(Address.Parse(genesisContractAddress), _account);
            var configurationAddress = await basicZeroStub.GetContractAddressByName.CallAsync(GenesisContract.NameProviderInfos[NameProvider.Configuration]);
            _logger.WriteInfo($"Configuration contract address: {configurationAddress.GetFormatted()}");
            
            var configurationStub = _stub.Create<ConfigurationContainer.ConfigurationStub>(configurationAddress, _account);

            return configurationStub;
        }

        private async Task SetSelectTransactionLimit(ConfigurationContainer.ConfigurationStub configurationStub, int limitCount)
        {
